...
async def ytdl_video(path, video_url, id, session):
    print(video_url)
...
async def ytdl_down(path, video_url, id, session):
    print(video_url)
    qa = "mp3"
    file = f"{path}/%(title)s"
    ydl_opts = {
...
async def _(Mbot, message):
    ...
    try:
        ...
        if LOG_GROUP:
            ...
            await message.reply(f"400: Sorry, Unable To Find It  try another or report it to @masterolic or support chat @DeadlineTechSupport 💬")
            await Mbot.send_message(LOG_GROUP, traceback.format_exc())
